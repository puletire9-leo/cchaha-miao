...
let isBenchmarking = false  // 测速中标志，防止重复测速
  ...
  for (const model of models) {
    ...
    try {
      ...
      if (result.status >= 200 && result.status < 300) {
        ...
        // 打印详细的错误响应以便调试
        const errorPreview = result.data ? result.data.substring(0, 100) : '无响应内容'
        console.log(`失败 ${result.status} - ${errorPreview}`)
...
async function handleRequest(req, res) {
  ...
  if (req.url === '/health') {
    ...
    res.end(JSON.stringify({ status: 'ok', models: models.length }))
    return
  }
